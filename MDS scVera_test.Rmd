...
source("~/1A_Jordan_Lab/monica-coding-help/test_data/cellsnp_functions.R")

# pulling in variants from cellsnp
p1236_cellsnp <- 
  read_cellsnp("~/1A_Jordan_Lab/monica-coding-help/test_data/p1236/")
## There will be multiple files here in the future so these could be loaded and
# manipulated into a list
...
p1236_subset_alt <-
  dplyr::mutate(
    p1236_frame_alt, 
    # p1236_frame_alt? (was p1236_frame)
    cells_per_variant = rowSums(p1236_frame_alt != 0)
    ) %>% 
  filter(cells_per_variant >10) %>% 
  select(-cells_per_variant)
...
capture_barcode_clusters <-
  read_table2(
    "~/1A_Jordan_Lab/monica-coding-help/test_data/capture_barcode_clusters.tsv"
    )
...
p1236_filtered_names <- 
  names(p1236_subset_alt)[
    (names(p1236_subset_alt) %in% p1236_barcode_clusters$barcode)
    ]
