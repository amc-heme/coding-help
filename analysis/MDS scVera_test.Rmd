...
source("~/Documents/JordanLab/snippets/cellsnp_functions.R")
# pulling in variants from cellsnp
p1236_cellsnp <- read_cellsnp("./raw_data/p1236")
...
p1236_subset_alt <-
  dplyr::mutate(p1236_frame_alt, cells_per_variant = rowSums(p1236_frame != 0)) %>%
  filter(cells_per_variant > 10) %>% select(-cells_per_variant)

#subsetting the ref list to match the alt list
p1236_subset_alt <-
  rownames_to_column(p1236_subset_alt, var = "location")
p1236_subset_ref <-
  rownames_to_column(p1236_frame_ref, var = "location")
p1236_subset_ref <-
  p1236_subset_ref[match(p1236_subset_alt$location, p1236_subset_ref$location), ]


p1236_subset_alt <-
  column_to_rownames(p1236_subset_alt, var = "location")
row.names(p1236_subset_ref) <- NULL
p1236_subset_ref <-
  column_to_rownames(p1236_subset_ref, var = "location")
...
capture_barcode_clusters <-
  read_table2("~/Desktop/scVar/mito/MDS_invivo_cluster_barcodes.tsv")
p1236_barcode_clusters <-
  filter(capture_barcode_clusters, capture == "htb-1236")
##yields 3568 barcodes in Krysta's Seurat object

# filtering the data to match the cell ID in the seurat object
p1236_filtered_names <-
  names(p1236_subset_alt)[(names(p1236_subset_alt) %in% p1236_barcode_clusters$barcode)]
...
remove <- c("barcode")
seurat_groups <-
  seurat_groups[!(row.names(seurat_groups) %in% remove),]
...
seurat_groups_filtered <-
  names(seurat_groups)[(names(seurat_groups) %in% filtered_barcodes)]
...
p1236_annotation <-
  select(metadata_list_MDS$p1236,
         mutation,
         variant,
         ExonicFunc.refGene)
p1236_filtered_alt <-
  rownames_to_column(p1236_filtered_alt, var = "variant")
data_p1236_alt <-
  full_join(p1236_annotation, p1236_filtered_alt, by = "variant")

# add full length mutation name to ref
p1236_filtered_ref <-
  rownames_to_column(p1236_filtered_ref, var = "variant")
data_p1236_ref <-
  full_join(p1236_annotation, p1236_filtered_ref, by = "variant")
...
p1236_RUNX1_alt <-
  data_p1236_alt %>% filter(variant == "chr21_34792108_G_T" |
                              variant == "cluster")
...
p1236_RUNX1_alt <-
  column_to_rownames(p1236_RUNX1_alt, var = "mutation") %>% select(-variant, -ExonicFunc.refGene)
...
p1236_RUNX1_alt_count <-
  filter(
    p1236_RUNX1_alt,
    `RUNX1_chr21.34792108.G.T` == "1" |
      `RUNX1_chr21.34792108.G.T` == "2" |
      `RUNX1_chr21.34792108.G.T` == "3" |
      `RUNX1_chr21.34792108.G.T` == "4"
  )
...
#RUNX1_ref
p1236_RUNX1_ref <-
  data_p1236_ref %>% filter(variant == "chr21_34792108_G_T" |
                              variant == "cluster")
p1236_RUNX1_ref[2, 1] <- "cluster"
p1236_RUNX1_ref <-
  column_to_rownames(p1236_RUNX1_ref, var = "mutation") %>% select(-variant, -ExonicFunc.refGene)
...
#Filter out the cells with counts
p1236_RUNX1_ref_count <-
  filter(
    p1236_RUNX1_ref,
    `RUNX1_chr21.34792108.G.T` == "1" |
      `RUNX1_chr21.34792108.G.T` == "2" | `RUNX1_chr21.34792108.G.T` == "3"
  )
...
p1236_RUNX1_alt_count <-
  rownames_to_column(p1236_RUNX1_alt_count, var = "mutation")
p1236_RUNX1_ref_count <-
  rownames_to_column(p1236_RUNX1_ref_count, var = "mutation")
#p1236_RUNX1_summary<-full_join(p1236_RUNX1_alt_count,p1236_RUNX1_ref_count, by="mutation")
p1236_RUNX1_overlap <-
  inner_join(p1236_RUNX1_alt_count, p1236_RUNX1_ref_count, by = "mutation")
